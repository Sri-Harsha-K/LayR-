...
- `projectStore.ts` — the serializable `Project` (tracks, clips, effects, mixer, scenes), wrapped in `zundo`'s `temporal` middleware so *only* this slice is undoable. This is what gets saved/loaded/exported. Multi-event pointer gestures (velocity drag, note move/resize) call `pauseHistory()`/`resumeHistory()` around the gesture so it collapses into one undo step instead of one per `pointermove` — do this for any new drag-based edit. `sanitizeProject()` (called from `loadProject`) is where an untrusted/older-version loaded project gets defaulted/clamped to current invariants (missing `scenes`, out-of-range `bpm`) — extend it rather than trusting `Project`'s type when reading from disk.
